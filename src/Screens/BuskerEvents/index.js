...
import BuskEventsRoute from './BuskEvents';
import UserEventsRoute from './MyEvents';

const routes = [
  {key: 'Busker', title: 'Busker Events'},
  {key: 'User', title: 'My Events'},
];
...
const Home = props => {
  ...
  const renderScene = ({route, jumpTo}) => {
    switch (route.key) {
      // case 'Busker':
      //   return <BuskerRoute navigation={props.navigation} />;
      // case 'User':
      //   return <UserRoute navigation={props.navigation} />;
      case 'Busker':
        return (
          <BuskEventsRoute navigation={props.navigation} toast={toast.current} />
        );
      case 'User':
        return (
          <UserEventsRoute
  ...
  // Busker Events Route UI Component
  const BuskerRoute = props => {
    return (
  ...
  // User Events Route UI Component
      ...
      <View style={styles.viewStyle}>
        ...
        {userEvents.length != 0 && (
          <FlatList
            ...
            onEndReached={({distanceFromEnd}) => {
              console.log(' ***************** ' + distanceFromEnd);
              if (eventPage <= totalEventPage && onEventLoad) {
                UserEventsApi(user);
              }
            }}
  ...
  return (
    <SafeAreaView style={styles.viewcontainer}>
      ...
      <View style={styles.headerRowStyle}>
        ...
        <Text style={styles.titleText}>{I18n.t('events.events')}</Text>
        <TouchableOpacity style={styles.backImageBack} />
      </View>
      <View style={styles.viewStyle}>
        <TabView
